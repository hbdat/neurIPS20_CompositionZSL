...
import pickle 
#%%
# this is a complex dataloader that I optimize the loading time by loading a chunk of data at a time
#%%
...
class DeepFashionDataLoader():
    def __init__(self, data_path, device, is_balance =True, is_scale = False,verbose = False,hdf5_template=None):
        ...
        if hdf5_template is None:
            hdf5_template = 'feature_map_ResNet_101_{}_sep_seen_samples.hdf5'
    ...
    def next_batch_img(self,batch_size):
        ...
        for i_c in sampled_idx_c:
            ...
            idx_sample_c = np.random.choice(n_samples_c,n_target_class).squeeze()
            
            l = self.seenclasses[sampled_idx_c].item()
            
            img_file = self.dict_train[l][idx_sample_c]
            img_file = os.path.join(img_dir,img_file)
            
            batch_feature.append(part_feature[idx_sample_c])
            batch_label.append(part_label[idx_sample_c])
            batch_files.append(img_file)
